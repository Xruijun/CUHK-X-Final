    ...
    with open(csv_path, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            if len(row) >= 3:  # Ensure row has enough columns
                ...
                # 跳过无效结果
                if result == 'OOM' or result.startswith('ERROR'):
...
if __name__ == "__main__":
    ...
    csv_path = f"CUHK-X-VLM/src/task_logic/predictions/{modality}/pred_{method}_new.csv"
    ...
    output_csv = f"CUHK-X-VLM/src/task_logic/scores/{modality}/{method}_accuracy_new.csv"
